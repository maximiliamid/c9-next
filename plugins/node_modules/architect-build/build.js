...
function compileLess(opts, sources, callback) {
    ...
    function readLibs(cb) {
        async.each(Object.keys(libs), function(key, next) {
            var lib = libs[key];
    ...
    function compile() {
        ...
        return lessParser.render(code, ctx, function(err, output) {
...
// toCss() was removed in the less@2 -> less@4 migration: less@3+ dropped the
// parse() + new less.ParseTree(tree, imports).toCSS() API in favor of the async
// less.render(input, options, cb) (returns { css }); compile() now calls render().
